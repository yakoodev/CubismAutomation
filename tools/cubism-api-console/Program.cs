    ...
    public static ApiCatalog Default()
    {
        return new ApiCatalog([
            ...
            new ApiGroup("jobs", "Jobs API", [
                ...
                new ApiAction("GET /jobs?status=done&limit=20", "GET", "/jobs?status=done&limit=20"),
                ...
                new ApiAction("DELETE /jobs/{id}", "DELETE", "/jobs/job-REPLACE_ME"),
                new ApiAction("POST /jobs/cleanup", "POST", "/jobs/cleanup", """{"status":"done","limit":100}""")
            ])
        ]);
    }
}
